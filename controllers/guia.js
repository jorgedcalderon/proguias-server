          ...
          Guia.findByIdAndUpdate(
            ...
            (err, userResult) => {
              if (err) {
                res
                  .status(500)
                  .send({ message: "Error del upload findbyidandupdate." });
              } else {
                if (!userResult) {
                  res
                    .status(404)
                  ...
                  res.status(200).send({ avatarName: req.file.filename });
                }
              }
            }
          );
...
function subirCompe(req, res) {
  ...
  if (req.file) {
    ...
    Guia.findOneAndUpdate(
      { _id: id, "certs.name": idCompe },
      {
